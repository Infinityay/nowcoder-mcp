...
| `tag` | int | ❌ | 818 | 标签筛选ID。可选值：`818`(面经)、`861`(求职进度)、`823`(内推)、`856`(公司评价)。`null`=不筛选 |
| `order` | string | ❌ | "create" | 排序方式。可选值：`""`(空字符串,默认排序)、`"create"`(按时间排序) |
...
```python
# 搜索 Java 后端相关面经，按时间排序
search(query="Java后端")

# 搜索面经，获取前 5 页结果
search(query="秋招经验", max_pages=5, tag=818, order="create")

# 搜索内推信息，默认排序
search(query="字节跳动", tag=823, order="")

# 搜索公司评价，按时间排序
search(query="阿里巴巴", tag=856, order="create")

# 不筛选标签，获取全部结果
search(query="算法工程师", max_pages=0, tag=None, order="create")
```
...
| `tag` | int | ❌ | null | 标签筛选ID。可选值：`818`(面经)、`861`(求职进度)、`823`(内推)、`856`(公司评价)。`null`=不筛选 |
| `order` | string | ❌ | "" | 排序方式。可选值：`""`(空字符串,默认排序)、`"create"`(按时间排序) |
...
# 批量搜索多个岗位的面经，每个获取 3 页，按时间排序
batch_search(keywords=["后端开发", "前端开发", "算法工程师"], max_pages=3, tag=818, order="create")

# 批量搜索内推信息
batch_search(keywords=["美团内推", "京东内推"], tag=823, order="create")
```
